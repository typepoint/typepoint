...
Note that TypePoint is currently undergoing a massive refactoring to split its concerns into mulitple packages. The below documentation is for version `0.11.0` of `@typepoint/core`.
...
export const GetTodo = new EndpointDefinition<{ id: string }, Empty, Todo>(path => path.literal('todos').param('id'));
```
...
client
  .fetch(GetTodo, {
    params: {
      // Params will be required and strongly typed
      id: '123',
    },
  })
  .then(response => {
