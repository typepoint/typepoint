...
describe('shared', () => {
  describe('EndpointDefinition', () => {
    ...
    it('should error if method is not supported', () => {
      ...
      expect(() => new EndpointDefinition<Empty, Empty, Product[]>(method as any, path => path.literal('products'))).toThrow('Unsupported HTTP method: SQUANCH');
    });
  ...
  describe('arrayOf', () => {
    ...
    it('should error if typeInfo is evaluated', () => {
      const ArrayOfUser = arrayOf(User);
      const arrayOfUser = new ArrayOfUser();
      expect(() => arrayOfUser.typeInfo()).toThrow('Do not evaluate ArrayOf.typeInfo(). It is reserved for internal use only.');
    });
  });
