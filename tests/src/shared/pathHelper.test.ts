...
describe('shared/pathHelper', () => {
  ...
  describe('url', () => {
    it('should generate the url correctly', () => {
      ...
      for (const testCase of testCases) {
        ...
        const url = helper.url(testCase.input.params);
        expect(url).to.deep.equal(testCase.expected);
      }
    });
  ...
  describe('parse', () => {
    it('should parse an url returning extracted values', () => {
      ...
      type ParseTestCase = TestCase<{ pattern: string, path: string }, { params: { [key: string]: any } }>;
